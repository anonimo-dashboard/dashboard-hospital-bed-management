...
media_total = df["taxa_ocupacao_total"].mean() * 100
mediana_total = df["taxa_ocupacao_total"].median() * 100
max_total = df["taxa_ocupacao_total"].max() * 100
p90_total = df["taxa_ocupacao_total"].quantile(0.90) * 100

dias_acima_80 = df[df["taxa_ocupacao_total"] >= 0.80].shape[0]
dias_acima_100 = df[df["taxa_ocupacao_total"] >= 1].shape[0]
...
cor_media = (
    COR_CRITICO if media_total >= 80
    else COR_ATENCAO if media_total >= 60
    else COR_ESTAVEL
)

cor_p90 = (
    COR_CRITICO if p90_total >= 100
    else COR_ATENCAO if p90_total >= 80
    else COR_ESTAVEL
)

cor_max = (
    COR_CRITICO if max_total >= 100
    else COR_ATENCAO if max_total >= 80
    else COR_ESTAVEL
)
...
if media_total >= 80:
    ...
    msg_status = "A média da ocupação total está acima da faixa de atenção."
elif media_total >= 60:
    status_total = "ATENÇÃO"
    msg_status = "A média da ocupação total exige monitoramento operacional."
else:
...
Análise operacional da taxa de ocupação hospitalar total ao longo do tempo.
""")

if status_total == "CRÍTICO":
    st.error(f"🔴 Status da ocupação total: {status_total} — {msg_status}")
elif status_total == "ATENÇÃO":
    st.warning(f"🟠 Status da ocupação total: {status_total} — {msg_status}")
else:
    st.success(f"🟢 Status da ocupação total: {status_total} — {msg_status}")

st.markdown("## Indicadores da ocupação total")

col1, col2, col3, col4 = st.columns(4)

with col1:
    st.markdown(
        card_kpi("Média", f"{media_total:.1f}%", "Taxa média no período.", cor_media),
        unsafe_allow_html=True
    )

with col2:
    st.markdown(
        card_kpi("Mediana", f"{mediana_total:.1f}%", "Ponto central da distribuição.", COR_ESTAVEL),
        unsafe_allow_html=True
    )

with col3:
    st.markdown(
        card_kpi("Percentil 90", f"{p90_total:.1f}%", "90% dos dias ficaram abaixo.", cor_p90),
        unsafe_allow_html=True
    )

with col4:
    st.markdown(
        card_kpi("Máximo", f"{max_total:.1f}%", "Maior taxa observada.", cor_max),
        unsafe_allow_html=True
    )
...
with col8:
    st.markdown(
        card_kpi("Indicador", "Ocupação total", "Leitura sistêmica da ocupação.", "#1F4E79"),
        unsafe_allow_html=True
    )
...
with col_tab:
    ...
    if dias_criticos.empty:
        ...
        dias_criticos["taxa_ocupacao_total"] = (
            dias_criticos["taxa_ocupacao_total"] * 100
        ).round(1)

        dias_criticos = dias_criticos.rename(
            columns={
                "data": "Data",
                "taxa_ocupacao_total": "Taxa (%)",
                "ocupacao_total": "Ocupação",
...
fig_shewhart, media_sh, desvio_sh, lsc_sh, lic_sh, pontos_fora = criar_grafico_shewhart(
    df=df,
    coluna_taxa="taxa_ocupacao_total",
    titulo="Carta exploratória — Ocupação Total"
)
...
st.info(
    f"A ocupação total apresentou média de {media_total:.1f}% e mediana de {mediana_total:.1f}%. "
    f"O percentil 90 foi de {p90_total:.1f}%, indicando que 90% dos dias ficaram abaixo desse patamar. "
    f"Foram registrados {dias_acima_80} dias acima de 80% e {dias_acima_100} dias com ocupação igual ou superior a 100%. "
    f"O pico observado foi de {max_total:.1f}%."
)

with st.expander("Nota metodológica"):
    st.markdown("""
    A ocupação hospitalar total representa uma leitura sistêmica da pressão sobre a capacidade disponível.
